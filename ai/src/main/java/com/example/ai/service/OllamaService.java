...
public class OllamaService {
    ...
    OllamaService(VectorStore vectorStore, ChatClient.Builder builder, ChatMemory chatMemory) {
        DOCS.stream().map(this::loadDocumentsFromFile).forEach(vectorStore);
    ...
    public String getAnswer(String userId, String question) {
        return this.chatClient.prompt().user(question).advisors(a -> a.param(ChatMemory.CONVERSATION_ID, userId)).call().content();
    }
}
